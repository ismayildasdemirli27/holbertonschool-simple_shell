...
char *_getenv(const char *name)
{
	...
	/* Əgər mühit dəyişənləri tamamilə silinibdirsə, çökmənin qarşısını al */
	if (!environ || !name)
		return (NULL);
...
char *get_location(char *command)
{
	...
	/* Komandada '/' varsa (məs: /bin/ls və ya ./ls) yalnız mövcudluğunu yoxla */
	if (strchr(command, '/'))
	{
	...
	/* '/' yoxdursa və PATH boşdursa, axtarışı dayandır (fork yaratma) */
	path = _getenv("PATH");
	if (!path || !*path)
		return (NULL);
...
void execute_cmd(char **args, char *argv0, char *line)
{
	...
	if (!cmd_path)
	{
		/* Komanda tapılmadısa səhv mesajı ver və FORK YARATMA */
		perror(argv0);
		return;
	}
	child_pid = fork();
